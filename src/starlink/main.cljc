(ns starlink.main
  "Kotodama WASM entrypoint for the Starlink clean-room actor (L4) — Clojure port.
  ...
  facts (`starlink.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "starlink")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"starlink.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "starlink-compat" :tier tier :entities entities} 200])
...
(defn start! [] :starlink-compat/ready)
